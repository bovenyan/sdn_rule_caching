#ifndef __TRACE_PARSER_H
#define __TRACE_PARSER_H
...
struct sniff_etherVPN {
        u_char  ether_dhost[ETHER_ADDR_LEN];    /* destination host address */
        u_char  ether_shost[ETHER_ADDR_LEN];    /* source host address */
        u_char  ether_vpn[ETHER_VPN_TAG];	/* VPNtag */
	u_short ether_type;                     /* IP? ARP? RARP? etc */
...
void ParseTrace(const char *, const char *, size_t = 0);
