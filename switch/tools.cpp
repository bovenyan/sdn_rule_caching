...
unsigned int maskIP(string ip_s, unsigned int mask){
	...
	for (unsigned int i = 0; i < 4; i++){
		ip_i += (boost::lexical_cast<int>(splitedStr[i]) << (24-8*i));
	}
...
void nettoi(string ip_s, size_t (& net_i)[2]){
	vector<string> splitedStr;
	boost::split(splitedStr, ip_s, boost::is_any_of("/"));
	net_i[1] = atoi(splitedStr[1].c_str());
	net_i[0] = maskIP(splitedStr[0], net_i[1]);
}

bool insubnet(const size_t (&packPred) [4], const size_t (&subnet)[2]){
	if (subnet[1] != 0){
		size_t maskAnd = (~size_t(0)) << (32 - subnet[1]);
		if (((packPred[0] & maskAnd) != subnet[0]) || ((packPred[1] & maskAnd) != subnet[0]))
			return false;
	}
	return true;
}
