...
//  
//
//  Created by Hongya Xing on 13-4-23.
...
typedef struct{
    ...
    port_t *        dst_port;
    char *          protocol;
    unsigned int             bsrc_ip[32];
    unsigned int             bdst_ip[32];
...
vector<struct pkt_hdr_t> hdr_vec;
...
int main(int argc, char * argv[]){
    ...
    int             rid = 1;
    ifstream        hdr_file(argv[2]);
    string          hdrline;
    int             hdr_id = 1;
    
    if(hdr_file.is_open()){
        while(getline(hdr_file, hdrline)){
            stringstream hdr_strs(hdrline);
            pkt_hdr_t pkt_hdr;
            char hdr_src_ip[10]="";
            char hdr_dst_ip[10]="";
            ...
            //printf("%d\n",hdr_src_ip); 
            hdr_strs >> hdr_src_ip >> hdr_dst_ip >> hdr_src_port >> hdr_dst_port >> hdr_protocol;
            hdr_strs.getline(match_str, MAX_STRING);
            ...
            //printf("here?"); 
            pkt_hdr.src_ip=atoi(hdr_src_ip);
            pkt_hdr.dst_ip=atoi(hdr_dst_ip);
            pkt_hdr.src_port=atoi(hdr_src_port);
            pkt_hdr.dst_port=atoi(hdr_dst_port);
            pkt_hdr.protocol=atoi(hdr_protocol);
            
            hdr_vec.push_back(pkt_hdr);
            hdr_id++; 
        }
    }
    ...
    for(it = rule_vec.begin() ; it < rule_vec.end() ; it++){
        ...
        *tmpDst = *it->dst_ip;
        ...
        for(int i=0; i<32; i++){
            it->bisrc_ip += (unsigned int) (bsrc_ip[i] << (31-i));
            
            
            it->bidst_ip += (unsigned int) (bdst_ip[i]<< (31-i));
        }
        printf("rule %d src: %u dst %u\n", it->id, it->bisrc_ip, it->bidst_ip);
        printf("src: %d.%d.%d.%d/%d, dst: %d.%d.%d.%d/%d\n", it->src_ip->buf[0],it->src_ip->buf[1],it->src_ip->buf[2],it->src_ip->buf[3],it->src_ip->pre_len,it->dst_ip->buf[0],it->dst_ip->buf[1],it->dst_ip->buf[2],it->dst_ip->buf[3],it->dst_ip->pre_len);  
...
void  matching(){
    ...
    vector<struct rule_t>::iterator rit;
    vector<struct pkt_hdr_t>::iterator hdrit;
    ofstream fout("matched_rules.txt");
    int isMatch=0;
    
    for(hdrit = hdr_vec.begin();hdrit<hdr_vec.end();hdrit++){
        fout << hdrit->id << ":";
        for(rit=rule_vec.begin();rit<rule_vec.end();rit++){
            if(strcmp(rit->protocol,"0x00")==0){
                isMatch=1;
            }
            else{
                char *temp_str;
                int int_protocol=(int)strtol(rit->protocol,&temp_str,16);
                if(int_protocol==hdrit->protocol){
                    isMatch=1;
                }
                else{isMatch=0;}
            }
            if(isMatch==1){
                if(rit->src_port->upper >= hdrit->src_port && rit->src_port->lower <= hdrit->src_port){
                    isMatch=1;
                }
                else{isMatch=0;}
            }
            if(isMatch==1){
                if(rit->dst_port->upper >= hdrit->dst_port && rit->dst_port->lower <= hdrit->dst_port){
                    isMatch=1;
                }
                else{isMatch=0;}
            }
            if(isMatch==1){
                unsigned int src_mask = 1<<(rit->src_ip->pre_len+1);
                src_mask=src_mask-1;
                src_mask=src_mask<<(32-rit->src_ip->pre_len);
	//	printf("%d\n",rit->bisrc_ip);
                if((rit->bisrc_ip&src_mask)==(hdrit->src_ip&src_mask)){
                    isMatch=1;
                }
                else{isMatch=0;}
            }
            if(isMatch==1){
                unsigned int dst_mask = 1<<(rit->dst_ip->pre_len+1);
                dst_mask=dst_mask-1;
                dst_mask=dst_mask<<(32-rit->dst_ip->pre_len);
                if((rit->bidst_ip&dst_mask)==(hdrit->dst_ip&dst_mask)){
                    isMatch=1;
                }
                else{isMatch=0;}
            }
...
void  compare(){
    ...
    for(it = rule_vec.begin() ; it < rule_vec.end() ; it++){
        ...
        for(it1 = rule_vec.begin() ; it1 < it ; it1++){
