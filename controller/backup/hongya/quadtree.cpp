...
// bo #include <ctype.h>
// bo #include <errno.h>
// bo #include <sys/types.h>
// bo #include <sys/socket.h>
// bo #include <netinet/in.h>
...
struct node
{
    ...
    struct node *child[4];//一个节点拥有的子节点
    struct node *parent;
    int count; 
    int ruleID[MAXRULES];
    int totalCount;
...
void printBucket(Node *root){
    ...
    //        fprintf(fp, "total num  below: %d  ", test->totalCount);
            ...
            fprintf(fp, "*/%d  rule num: %d  ", test->masklen, test->count);
        ...
        for(m=0;m<4;m++){
            if(test->child[m]!=NULL){
                queue.push_back(test->child[m]);
            }
        }
