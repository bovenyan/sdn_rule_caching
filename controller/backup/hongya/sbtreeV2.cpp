     ...
     //       printf("rule %d pass d port\n", parent->ruleID[i]);
            if(rule->src_ip->pre_len <= child->masklen){
                for(int m=0;m<rule->src_ip->pre_len; m++){
                    if(rule->bisrc_ip[m]!=(((child->src >> (31-m)) & 1 ))){
                        isOverlap=0;
                    }
                }
            }
      ...
      //      printf("rule %d pass s ip\n", parent->ruleID[i]);
            if(rule->dst_ip->pre_len <= child->masklen){
                for(int m=0;m<rule->dst_ip->pre_len; m++){
                    if(rule->bidst_ip[m]!=(((child->dst >> (31-m)) & 1 ))){
                        isOverlap=0;
                    }
                }
            }
    ...
    if(flag == 0){
                ...
                for(int k=0; k<2;k++){
                    for(int l=0; l<2;l++){
                        ...
                        newnode->masklen=currnt->masklen+1;
                            ...
                            newnode->dst=currnt->dst;
                            newnode->src=currnt->src;
                  
                     
                        newnode->src += i << (32-newnode->masklen);
                        newnode->dst += j << (32-newnode->masklen);
                        
                        
                        int p= k << (16-newnode->masklen);
                        int q= l << (16-newnode->masklen);
                        
                        printf("%d, zuoyi\n", p);
                        printf("%d, power\n", power(2,15-newnode->masklen));
                    
                        
                        newnode->srcp = currnt-> srcp + p;
                        newnode->dstp = currnt-> dstp + q;
                        
                        
                        newnode->src_port_low = newnode->srcp;
                        newnode->src_port_up = newnode->src_port_low + power(2, 16-newnode->masklen)-1;
                        
                    
                        

                        newnode->dst_port_low = newnode->dstp;
        ...
        for(int i=0; i<2; i++){
            for(int j=0; j<2;j++){
                        ...
                        newnode->dst=currnt->dst;
                        newnode->src=currnt->src;
                        newnode->dstp=currnt->dstp;
                        newnode->srcp=currnt->srcp;
                ...
                int k= (currnt->srcp >> (15-currnt->masklen)) & 1;
                int l= (currnt->dstp >> (15-currnt->masklen)) & 1;
...
void compare(Node * node, int count){
    ...
    if(currnt->count > BUCKET_SIZE){
        ...
        if(currnt-> masklen <16){
            ...
            int k= currnt->srcp >> (15-currnt->masklen);
            int l= currnt->dstp >> (15-currnt->masklen);
            
            for(int i=0;i<2;i++){
                for(int j=0;j<2;j++){
                    if(currnt->child[i][j][k][j]);
                }
            }
    ...
    while(!queue.empty()){
        ...
        if(test->count>0){
            ...
            for(int i=0;i<test->masklen;i++){
                fprintf(fp, "%d", ((test->src >> (31-i)) & 1));
            }         
            fprintf(fp, "*/%d   dst prefix: ", test->masklen);
            for(int i=0;i<test->masklen;i++){
                fprintf(fp, "%d", ((test->dst >> (31-i)) & 1));
            }
        ...
        if(test->masklen<16){
            ...
            int k= (test->srcp >> (15-test->masklen)) & 1;
            int l= (test->dstp >> (15-test->masklen)) & 1;
